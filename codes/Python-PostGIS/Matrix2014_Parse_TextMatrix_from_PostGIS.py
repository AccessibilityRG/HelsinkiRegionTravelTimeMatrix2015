...
outDir = r"E:\MatriisiAjot2014\RESULTS\HelsinkiRegion_TravelTimeMatrix2014"
...
for index, row in ykr.iterrows():
    if index >= start_idx and index < end_idx:
        # Get to_id
        to_id = row['YKR_ID']
        print("Processing ID/index: %s, %s" % (to_id, index)) 

        # Get unique 'to_id' values from the db
        sql = """SELECT * FROM %s
                   WHERE to_id = %s;""" % (TABLE_NAME, to_id)
        ...
        # Select output data
        datacols = ['from_id', 'to_id', 'walk_t', 'walk_d', 'pt_m_tt', 'pt_m_t', 'pt_m_d', 'car_m_t', 'car_m_d']
        data = data[datacols]
