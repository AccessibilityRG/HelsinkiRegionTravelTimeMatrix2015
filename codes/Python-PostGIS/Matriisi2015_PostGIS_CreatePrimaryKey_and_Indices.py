...
def main():
    ...
    primary_key = 'id'
    createPrimaryKey(conn, cursor, DATA_TABLE, col_name=primary_key)
    setPrimaryKeyCol(conn, cursor, table=DATA_TABLE, key_column=primary_key)

    # Create indices to speed up queries
    createIndex(conn, cursor, DATA_TABLE, column='from_id', index_col='from_idx')
    createIndex(conn, cursor, DATA_TABLE, column='to_id', index_col='to_idx')

    # Vacuum table
    vacuumTable(conn, cursor, DATA_TABLE)
